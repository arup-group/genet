...
def test_save_to_geojson(tmpdir):
    ...
    assert nodes.loc['0', 'geometry'].coords[:] == [(-0.14625948709424305, 51.52287873323954)]
    assert nodes.loc['1', 'geometry'].coords[:] == [(-0.14478238148334213, 51.523754629002234)]
    assert links.loc[0, 'geometry'].coords[:] == [(-0.14625948709424305, 51.52287873323954),

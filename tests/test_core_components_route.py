...
def test_initiating_route(assert_semantically_equal, simple_route):
    r = simple_route
...
def test_updating_route_trips_with_headway(assert_semantically_equal, simple_route):
    simple_route.generate_trips_from_headway(
...
def test_generating_trips_from_headway_preserves_graph_schema(simple_route):
    verify_graph_schema(simple_route.graph())
...
def test__repr__shows_stops_and_trips_length(simple_route):
    assert str(len(simple_route.ordered_stops)) in simple_route.__repr__()
    assert str(len(simple_route.trips["trip_id"])) in simple_route.__repr__()


def test__str__shows_info(simple_route):
    assert simple_route.id in simple_route.__str__()
    assert simple_route.route_short_name in simple_route.__str__()


def test_print_shows_info(mocker, simple_route):
    mocker.patch.object(Route, "info")
    simple_route.print()
    Route.info.assert_called_once()


def test_info_shows_id_name_and_len_of_stops_and_trips(simple_route):
    info = simple_route.info()
    assert simple_route.id in info
    assert simple_route.route_short_name in info
    assert str(len(simple_route.ordered_stops)) in info
    assert str(len(simple_route.trips["trip_id"])) in info


def test_plot_delegates_to_util_plot_plot_graph_routes(mocker, simple_route):
    mocker.patch.object(plot, "plot_geodataframes_on_kepler_map")
    simple_route.plot()
    plot.plot_geodataframes_on_kepler_map.assert_called_once()


def test_plot_saves_to_the_specified_directory(tmpdir, simple_route):
    filename = f"route_{simple_route.id}_map"
    expected_plot_path = os.path.join(tmpdir, filename + ".html")
    assert not os.path.exists(expected_plot_path)

    simple_route.plot(output_dir=tmpdir)
...
def test_route_isin_exact_list(simple_route):
    a = simple_route
    assert a.isin_exact([simple_route, simple_route, simple_route])


def test_route_is_not_in_exact_list(simple_route, stop_epsg_27700):
    a = Route(
        route_short_name="route",
        mode="bus",
        stops=[stop_epsg_27700, stop_epsg_27700],
    ...
    assert not a.isin_exact([simple_route, simple_route, simple_route])
...
def test_is_strongly_connected_with_not_strongly_connected_route(simple_route):
    assert not simple_route.is_strongly_connected()
...
def test_has_self_loops_with_non_looping_route(simple_route):
    assert not simple_route.has_self_loops()


def test_has_more_than_one_stop_with_regular_route(simple_route):
    assert simple_route.has_more_than_one_stop()
...
def test_has_network_route_with_route_that_has_a_network_route(simple_route):
    assert simple_route.has_network_route()


def test_has_network_route_with_route_without_a_network_route(simple_route):
    simple_route.route = []
    assert not simple_route.has_network_route()
...
def test_has_valid_offsets_with_valid_route(simple_route):
    assert simple_route.has_valid_offsets()


def test_has_valid_offsets_with_route_with_invalid_offsets(simple_route):
    simple_route.departure_offsets = []
    simple_route.arrival_offsets = []
    assert not simple_route.has_valid_offsets()


def test_has_valid_offsets_with_route_with_wrong_number_of_offsets(simple_route):
    simple_route.departure_offsets = ["00:00:00", "00:03:00", "00:07:00"]
    simple_route.arrival_offsets = ["00:00:00", "00:03:00", "00:07:00"]
    assert not simple_route.has_valid_offsets()


def test_has_valid_offsets_with_route_with_empty_offsets(simple_route):
    simple_route.departure_offsets = []
    simple_route.arrival_offsets = []
    assert not simple_route.has_valid_offsets()


def test_has_id(simple_route):
    assert simple_route.has_id()


def test_is_valid_with_valid_route(simple_route):
    assert simple_route.is_valid_route()
...
def test_is_valid_with_non_network_route(simple_route):
    simple_route.route = []
    assert not simple_route.is_valid_route()
...
def test_building_trips_dataframe_with_stops_accepts_backwards_compatibility(
    assert_logging_warning_caught_with_message_containing, simple_route, mocker, caplog
):
    mocker.patch.object(Route, "trips_with_stops_to_dataframe")
    simple_route.trips_with_stops_to_dataframe(simple_route.trips_to_dataframe())
    simple_route.trips_with_stops_to_dataframe.assert_called_once()
    assert_logging_warning_caught_with_message_containing(
        caplog, "`route_trips_with_stops_to_dataframe` method is deprecated"
    )


def test_building_trips_dataframe_with_stops(simple_route):
    df = simple_route.trips_with_stops_to_dataframe()
...
def test_generating_trips_dataframe(simple_route):
    df = simple_route.trips_to_dataframe()
...
def test_vehicles(simple_route):
    assert simple_route.vehicles() == {"veh_1_bus", "veh_2_bus"}


def test_service_attribute_data_under_keys_throws_error_from_route_object(simple_route):
    with pytest.raises(ServiceIndexError):
        simple_route.service_attribute_data(keys=["name"])


def test_route_attribute_data_under_key(simple_route):
    df = simple_route.route_attribute_data(keys="route_short_name").sort_index()
    assert_frame_equal(df, DataFrame({"route_short_name": {"1": "name"}}))


def test_route_attribute_data_under_keys(simple_route):
    df = simple_route.route_attribute_data(keys=["route_short_name", "mode"]).sort_index()
    assert_frame_equal(df, DataFrame({"route_short_name": {"1": "name"}, "mode": {"1": "bus"}}))


def test_stop_attribute_data_under_key(simple_route):
    df = simple_route.stop_attribute_data(keys="x").sort_index()
    assert_frame_equal(df, DataFrame({"x": {"1": 4.0, "2": 1.0, "3": 3.0, "4": 7.0}}))


def test_stop_attribute_data_under_keys(simple_route):
    df = simple_route.stop_attribute_data(keys=["x", "y"]).sort_index()
    assert_frame_equal(
        df,
        DataFrame(
            {

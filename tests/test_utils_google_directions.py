...
def generated_request():
    nodes = {
        "107316": {"lat": 51.5188864, "lon": -0.1369442},
    ...
    response._content = json.dumps(
        {
            ...
            "routes": [
                {
                    "bounds": {
                        "northeast": {"lat": 51.5208376, "lng": -0.1369381},
                        "southwest": {"lat": 51.5188908, "lng": -0.1391098},
                    },
                    ...
                    "legs": [
                        {
                            ...
                            "duration_in_traffic": {"text": "1 min", "value": 35},
                            "end_address": "65 Cleveland St, Fitzrovia, London W1T 4JZ, UK",
                            "end_location": {"lat": 51.5208376, "lng": -0.1391098},
                            "start_address": "49 Newman St, Fitzrovia, London W1T 3DZ, UK",
                            "start_location": {"lat": 51.5188908, "lng": -0.1369381},
                            "steps": [
                                {
                                    "distance": {"text": "0.3 km", "value": 264},
                                    "duration": {"text": "1 min", "value": 71},
                    ...
                    "overview_polyline": {"points": "ahmyHzvYkCvCuCdDcBrB"},
    ...
    response._content = json.dumps(
        {
            ...
            "routes": [
                {
                    "bounds": {
                        "northeast": {"lat": 51.5193916, "lng": -0.1356769},
                        "southwest": {"lat": 51.5188908, "lng": -0.1369381},
                    },
                    "copyrights": "Map data ©2020 Google",
                    "legs": [
                        {
                            ...
                            "end_location": {"lat": 51.5188908, "lng": -0.1369381},
                            "start_address": "42 Charlotte St, Fitzrovia, London W1T 2NP, UK",
                            "start_location": {"lat": 51.5193916, "lng": -0.1356769},
                            "steps": [
                                {
                                    "distance": {"text": "0.1 km", "value": 104},
                                    "duration": {"text": "1 min", "value": 44},
                                    ...
                                    "polyline": {"points": "ekmyH~nYRp@Rp@?@DLRr@Rp@L`@"},
                                    "start_location": {"lat": 51.5193916, "lng": -0.1356769},
                                    "travel_mode": "DRIVING",
                                }
                            ],
                    ...
                    "overview_polyline": {"points": "ekmyH~nYbBzFblahblah"},
...
def test_send_requests_for_road_network(
    mocker, tmpdir, generated_request, google_directions_api_response
):
    mocker.patch.object(
        google_directions,
        "generate_requests",
    ...
    mocker.patch.object(
        ...
        "send_requests",
        return_value={
            ("107316", "107352"): {**generated_request, **{"request": Future(), "timestamp": 12345}}
        },
    )
    ...
    google_directions.send_requests.assert_called_once_with(
        google_directions.generate_requests.return_value, None, None, None, None, None
    )
...
def test_queries_build_correctly_without_traffic_model_specified():
    ...
    assert (
        result.url
        == "https://maps.googleapis.com/maps/api/directions/json?origin=1%2C2&destination=3%2C4&key=super_awesome_key&departure_time=now"
    )
...
def test_queries_build_correctly_with_optimistic_traffic_model():
    ...
    assert (
        result.url
        == "https://maps.googleapis.com/maps/api/directions/json?origin=1%2C2&destination=3%2C4&key=super_awesome_key&traffic_model=optimistic&departure_time=now"
    )
...
def test_queries_build_correctly_with_correct_unix_time():
    ...
    assert (
        result.url
        == "https://maps.googleapis.com/maps/api/directions/json?origin=1%2C2&destination=3%2C4&key=super_awesome_key&departure_time=1893506400"
    )
...
def test_generating_requests_on_simplified_graphs(assert_semantically_equal):
    ...
    n.apply_attributes_to_nodes(
        {
            1: {"x": 528915.9309752393, "y": 181899.48948011652},
    ...
    n.apply_attributes_to_nodes(
        {
            1: {"lon": -0.14327038749428384, "lat": 51.52130909540579},
...
def test_sending_requests(assert_semantically_equal, mocker, google_directions_api_response):
    ...
    mocker.patch.object(
        google_directions, "make_request", return_value=google_directions_api_response
    )
    ...
    assert_semantically_equal(
        api_requests,
        {
            (1, 10): {
                ...
                "origin": {"lat": 1, "lon": 2},
                "destination": {"lat": 1, "lon": 2},
                "request": google_directions_api_response,
                "timestamp": 12345,
            },
            (1, 3): {
                "path_nodes": [1, 2, 3],
...
def test_parsing_routes_with_a_good_response(
    ...
    data = google_directions.parse_routes(
        google_directions_api_response, generated_request["path_polyline"]
    )
    assert_semantically_equal(
        data, {"google_speed": expected_speed, "google_polyline": "ahmyHzvYkCvCuCdDcBrB"}
    )
...
def test_parsing_routes_with_a_response_without_traffic_info(
    ...
    data = google_directions.parse_routes(
        google_directions_api_response_without_traffic_info, generated_request["path_polyline"]
    )
    assert_semantically_equal(
        data, {"google_speed": expected_speed, "google_polyline": "ahmyHzvYkCvCuCdDcBrB"}
    )
...
def test_parsing_routes_with_multiple_legs_response(
    ...
    data = google_directions.parse_routes(
        google_directions_api_response_multiple_legs, generated_request["path_polyline"]
    )
    assert_semantically_equal(
        data, {"google_speed": expected_speed, "google_polyline": "ekmyH~nYbBzFblahblah"}
    )
...
def test_parsing_routes_with_a_bad_response(
    ...
    with caplog.at_level(logging.INFO):
        data = google_directions.parse_routes(
            request_denied_google_directions_api_response, generated_request["path_polyline"]
        )
    ...
    assert_semantically_equal(
        api_requests,
        {
            ("107316", "107352"): {
                ...
                "path_polyline": "ahmyHzvYGJyBbCGHq@r@EDIJGBu@~@SToAzAEFEDIJ",
                "origin": {"lat": 51.5188864, "lon": -0.1369442},
                "destination": {"lat": 51.5208299, "lon": -0.1391027},
                "timestamp": 12345,
                "parsed_response": {
                    "google_speed": 7.542857142857143,
                    "google_polyline": "ahmyHzvYkCvCuCdDcBrB",
                },
...
def test_mapping_results_to_edges_with_singular_route_data(assert_semantically_equal):
    ...
    assert_semantically_equal(
        google_edge_data,
        {
            ...
            ("4307345495", "4307345497"): {
                "google_speed": 3.7183098591549295,
                "google_polyline": "ahmyHzvYkCvCuCdDcBrB",
            },
            ("2440651556", "2440651552"): {
                "google_speed": 3.7183098591549295,
                "google_polyline": "ahmyHzvYkCvCuCdDcBrB",
            },
            ("107317", "4307345495"): {
...
def test_saved_results_appear_in_directory(tmpdir, generated_request):
    ...
    api_requests[o_d]["request"] = "request"
    api_requests[o_d]["timestamp"] = 12345
    api_requests[o_d]["parsed_response"] = {
        "google_speed": 3.7183098591549295,
        "google_polyline": "ahmyHzvYkCvCuCdDcBrB",
    }

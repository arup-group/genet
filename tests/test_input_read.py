...
def test_reading_json(assert_semantically_equal):
    ...
    assert_semantically_equal(
        dict(n.nodes()),
        {
            "25508485": {
                "x": 528489.467895946,
                "y": 182206.20303669575,
                "lat": 51.52416253323928,
                "lon": -0.14930198709481451,
                "s2_id": 5.221390301001263e18,
                "id": "25508485",
            },
            "21667818": {
                "x": 528504.1342843144,
                "y": 182155.7435136598,
                "lat": 51.523705733239396,
                "lon": -0.14910908709500162,
                "s2_id": 5.221390302696205e18,
                "id": "21667818",
            },
            "295927764": {
                "x": 528358.873300625,
                "y": 182147.63137142852,
                "lat": 51.52366583323935,
                "lon": -0.15120468709594614,
                "s2_id": 5.221390299830573e18,
                "id": "295927764",
            },
            "200048": {
                "x": 528364.6182191424,
                "y": 182094.16422237465,
                "lat": 51.52318403323943,
                "lon": -0.1511413870962065,
                "s2_id": 5.221390299873156e18,
                "id": "200048",
            },
        },
    )
    ...
    assert_semantically_equal(
        ...
        {
            "name": "Schedule graph",
            "route_to_service_map": {"VJ6c64ab7b477e201cae950efde5bd0cb4e2e8888e": "20274"},
            ...
            "routes": {
                "VJ6c64ab7b477e201cae950efde5bd0cb4e2e8888e": {
                    ...
                    "mode": "bus",
                    "trips": {
                        "trip_id": ["VJ4e00b97ca9c6c0c96da8f793dfbd37b11f647fa7_03:40:00"],
            ...
            "services": {"20274": {"id": "20274", "name": "N55"}},
            "crs": "epsg:27700",
        },
    )
    assert_semantically_equal(
...
def test_reading_geojson(assert_semantically_equal):
    ...
    links = dict(n.links())
    assert [(round(x, 6), round(y, 6)) for x, y in links["1"]["geometry"].coords] == [
        (528489.467896, 182206.203037),
        (528504.134284, 182155.743514),
    ]
    assert [(round(x, 6), round(y, 6)) for x, y in links["10"]["geometry"].coords] == [
        (528364.618219, 182094.164222),
        (528358.873301, 182147.631371),
    ]
...
def test_read_gtfs_returns_expected_schedule(
    ...
    assert schedule["1001"] == Service(
        ...
        [
            Route(
                ...
                mode="bus",
                stops=[
                    Stop(id="BSE", x=-0.1413621, y=51.5226864, epsg="epsg:4326"),
    ...
    assert schedule["1002"] == Service(
        ...
        [
            Route(
                ...
                mode="rail",
                stops=[
                    Stop(id="RSN", x=-0.1410946, y=51.5231335, epsg="epsg:4326"),
    ...
    assert_semantically_equal(
        schedule.stop_to_service_ids_map(), correct_stops_to_service_mapping_from_test_gtfs
    )
    assert_semantically_equal(
        schedule.stop_to_route_ids_map(), correct_stops_to_route_mapping_from_test_gtfs
    )

...
def test_assume_travel_modes_doesnt_assume_modes_for_construction_highway_tags(
    full_fat_default_config,
):
...
def test_find_matsim_link_values_finds_values_for_well_defined_highway_osm_tag(
    ...
    assert_semantically_equal(
        matsim_vals, {"permlanes": 2.0, "freespeed": 33.36, "capacity": 2000.0}
    )
...
def test_find_matsim_link_values_defaults_to_highway_secondary_if_unknown_highway_osm_tag(
    ...
    matsim_vals = osm_reader.find_matsim_link_values(edge_data, full_fat_default_config)
    assert_semantically_equal(
        matsim_vals, {"permlanes": 1.0, "freespeed": 16.68, "capacity": 1000.0}
    )
...
def test_find_matsim_link_values_finds_values_for_non_highway_osm_tag(
    ...
    matsim_vals = osm_reader.find_matsim_link_values(edge_data, full_fat_default_config)
    assert_semantically_equal(
        matsim_vals, {"permlanes": 1.0, "freespeed": 44.44, "capacity": 9999.0}
    )
...
def test_find_matsim_link_values_to_highway_secondary_values_if_surrounded_with_gibberish_modes(
    ...
    edge_data = {
        "osmid": 0,
        "modes": ["piggyback", "walk", "levitating"],
        "length": 1748.4487354464366,
    }
    matsim_vals = osm_reader.find_matsim_link_values(edge_data, full_fat_default_config)
    assert_semantically_equal(
...
def test_generate_graph_nodes(assert_semantically_equal):
    nodes = {
        0: {
            "osmid": 0,
            "s2id": 1152921492875543713,
            "x": 0.008554364250688652,
            "y": -0.0006545205888310243,
        },
        1: {
            "osmid": 1,
            "s2id": 1152921335974974453,
            "x": 0.024278505899735615,
            "y": -0.0006545205888310243,
        },
        2: {
            "osmid": 2,
            "s2id": 384307157539499829,
            "x": -0.00716977739835831,
            "y": -0.0006545205888310243,
        },
    }
    ...
    assert_semantically_equal(
        generated_nodes,
        {
            "0": {
                ...
                "x": 623528.0918284899,
                "y": -5527136.199112928,
                "lat": -0.0006545205888310243,
                "lon": 0.008554364250688652,
                "s2_id": 1152921492875543713,
            },
            "1": {
                "id": "1",
                "x": 625278.7312853877,
                "y": -5527136.1998170335,
                "lat": -0.0006545205888310243,
                "lon": 0.024278505899735615,

...
def network_for_summary_stats():
    ...
    n.add_link(
        ...
        "0",
        "1",
        attribs={"length": 123, "modes": ["bike"], "attributes": {"osm:way:highway": "secondary"}},
    )
...
def test_reproject_changes_x_y_values_for_all_nodes(assert_semantically_equal, network1):
    ...
    target_change_log = pd.DataFrame(
        {
            "timestamp": {3: "2020-07-09 19:50:51", 4: "2020-07-09 19:50:51"},
            "change_event": {3: "modify", 4: "modify"},
            "object_type": {3: "node", 4: "node"},
            "old_id": {3: "101982", 4: "101986"},
            "new_id": {3: "101982", 4: "101986"},
            "old_attributes": {
            ...
            "diff": {
                3: [
                    ("change", "x", ("528704.1425925883", -0.14625948709424305)),
                    ("change", "y", ("182068.78193707118", 51.52287873323954)),
                ],
                4: [
                    ("change", "x", ("528835.203274008", -0.14439428709377497)),
    ...
    for i in [3, 4]:
        assert_semantically_equal(
            ast.literal_eval(target_change_log.loc[i, "old_attributes"]),
            ast.literal_eval(network1.change_log.loc[i, "old_attributes"]),
        )
        assert_semantically_equal(
            ast.literal_eval(target_change_log.loc[i, "new_attributes"]),
            ast.literal_eval(network1.change_log.loc[i, "new_attributes"]),
        )
    cols_to_compare = ["change_event", "object_type", "old_id", "new_id", "diff"]
    assert_frame_equal(
        network1.change_log[cols_to_compare].tail(2),
        target_change_log[cols_to_compare],
        check_dtype=False,
    )
...
def test_reprojecting_links_with_geometries():
    n = Network("epsg:27700")
    n.add_nodes(
        {"A": {"x": -82514.72274, "y": 220772.02798}, "B": {"x": -82769.25894, "y": 220773.0637}}
    )
    n.add_links(
        {
            "1": {
...
def test_adding_disjoint_networks_with_unique_ids_results_in_distinct_data_together(
    ...
    n_right = Network("epsg:27700")
    n_right.add_node("10", {"id": "10", "x": 1, "y": 1, "lon": 1, "lat": 1, "s2_id": 1})
    n_right.add_node("20", {"id": "20", "x": 1, "y": 1, "lon": 1, "lat": 1, "s2_id": 2})
    ...
    assert_semantically_equal(
        dict(n_left.nodes()),
        {
            "10": {"id": "10", "x": 1, "y": 1, "lon": 1, "lat": 1, "s2_id": 1},
    ...
    assert_semantically_equal(
        dict(n_left.links()),
        {
            "100": {"modes": ["walk"], "from": "10", "to": "20", "id": "100", "length": 145},
            "1": {"modes": ["walk"], "from": "1", "to": "2", "id": "1", "length": 145},
        },
    )
...
def test_adding_disjoint_networks_with_clashing_ids():
    ...
    n_right = Network("epsg:27700")
    n_right.add_node("1", {"id": "1", "x": 1, "y": 1, "lon": 1, "lat": 1, "s2_id": 1})
    n_right.add_node("2", {"id": "2", "x": 1, "y": 1, "lon": 1, "lat": 1, "s2_id": 2})
    n_right.add_link("1", "1", "2", attribs={"modes": ["walk"], "length": 145})
...
def puma_network():
    return read.read_matsim(
        path_to_network=puma_network_test_file,
        epsg="epsg:27700",
...
def network_with_simplified_schema():
    ...
    n.add_link(
        ...
        attribs={
            ...
            "geometry": LineString(
                [
                    (528704.1425925883, 182068.78193707118),
                    (528754.425925883, 182038.78193707118),
...
def test_simplify_does_not_oversimplify_PT_endpoints(
    puma_network_with_pt_stops_at_risk_of_oversimplification,
):
...
def test_simplified_network_saves_to_correct_dtds(
    tmpdir, network_dtd, network_with_simplified_schema
):
...
def test_reading_back_simplified_network():
    ...
    n = read.read_matsim(
        path_to_network=simplified_network, epsg="epsg:27700", path_to_schedule=simplified_schedule
    )

    number_of_simplified_links = 659

    links_with_geometry = n.extract_links_on_edge_attributes(
        conditions={"geometry": lambda x: True}
    )
    ...
    for link in links_with_geometry:
        ...
        if "attributes" in attribs:
            assert "geometry" not in attribs["attributes"]
...
def test_node_attribute_data_under_keys(network1):
    ...
    df_to_compare = pd.DataFrame(
        {
            "x": {"101982": "528704.1425925883", "101986": "528835.203274008"},
...
def test_link_attribute_data_under_keys_with_named_index(network1):
    df = network1.link_attribute_data_under_keys(
        ["modes", "freespeed", "capacity", "permlanes"], index_name="index"
    )
    assert df.index.name == "index"
...
def test_adding_nodes_with_mismatched_spatial_attribs_gets_filled_in():
    ...
    n.add_nodes(
        {
            1: {"lat": 51.521719064780775, "lon": -0.13777870665428316},
...
def test_adding_multiple_edges_between_same_nodes():
    n = Network("epsg:27700")
    n.add_edges(
        [{"from": 1, "to": 2}, {"from": 1, "to": 2}, {"from": 1, "to": 2}, {"from": 2, "to": 3}]
    )
    assert n.graph.has_edge(1, 2)
...
def test_adding_multiple_links_with_id_clashes(assert_semantically_equal):
    ...
    assert_semantically_equal(
        links_and_attribs[reindexing_dict["0"]], {"from": 1, "to": 2, "id": reindexing_dict["0"]}
    )
...
def test_adding_multiple_links_with_multiple_id_clashes(assert_semantically_equal):
    ...
    assert_semantically_equal(
        links_and_attribs[reindexing_dict["1"]], {"from": 2, "to": 3, "id": reindexing_dict["1"]}
    )
...
def test_adding_loads_of_multiple_links_between_same_nodes(assert_semantically_equal):
    ...
    assert_semantically_equal(
        links_and_attribs, {i: {"from": 1, "to": 2, "id": i} for i in range(10)}
    )
    assert_semantically_equal(
        n.link_id_mapping, {i: {"from": 1, "to": 2, "multi_edge_idx": i} for i in range(10)}
    )
...
def test_adding_multiple_links_with_id_and_multi_idx_clashes(assert_semantically_equal):
    ...
    assert_semantically_equal(
        n.link_id_mapping[reindexing_dict["0"]], {"from": 1, "to": 2, "multi_edge_idx": 2}
    )
    assert_semantically_equal(
        n.link_id_mapping[reindexing_dict["1"]], {"from": 1, "to": 2, "multi_edge_idx": 3}
    )
...
def test_adding_multiple_links_missing_some_from_nodes():
    ...
    assert "You are trying to add links which are missing `from` (origin) nodes" in str(
        error_info.value
    )
...
def test_adding_multiple_links_missing_some_to_nodes():
    ...
    assert "You are trying to add links which are missing `to` (destination) nodes" in str(
        error_info.value
    )
...
def test_adding_multiple_links_to_same_edge_clashing_with_existing_edge(assert_semantically_equal):
    ...
    assert_semantically_equal(
        n.link_id_mapping,
        {
            "0": {"from": "2", "to": "2", "multi_edge_idx": 0},
...
def test_nodes_on_spatial_condition_with_s2_region(network_object_from_test_data):
    ...
    network_object_from_test_data.add_node(
        "1",
        {
            ...
            "x": 508400,
            "y": 162050,
            "s2_id": spatial.generate_index_s2(51.3472033, 0.4449167),
        },
    )
    nodes = network_object_from_test_data.nodes_on_spatial_condition(region)
    assert set(nodes) == {"21667818", "25508485"}
...
def test_links_on_spatial_condition_with_intersection_and_complex_geometry_that_falls_outside_region(
    ...
    network_object_from_test_data.add_link(
        ...
        attribs={
            "geometry": LineString(
                [
                    (528504.1342843144, 182155.7435136598),
                    (508400, 162050),
                    (528489.467895946, 182206.20303669578),
                ]
            )
        },
    )
...
def test_subnetwork_on_spatial_condition_delagates_to_spatial_methods_to_get_subset_items(
    ...
    Schedule.services_on_spatial_condition.assert_called_once_with(
        region_input="region", how="intersect"
    )
    Network.links_on_spatial_condition.assert_called_once_with(
        region_input="region", how="intersect"
    )
    Network.subnetwork.assert_called_once_with(
        links={"link"},
        services={"service"},
...
def test_add_link_generates_new_multi_idx_if_already_exists():
    ...
    assert n.graph[1][2][n.link_id_mapping["1"]["multi_edge_idx"]] == {
        "from": 1,
        "to": 2,
        "id": "1",
    }
...
def test_reindex_node(network1):
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {
                3: "2020-06-08 19:39:08",
                4: "2020-06-08 19:39:08",
                5: "2020-06-08 19:39:08",
            },
            "change_event": {3: "modify", 4: "modify", 5: "modify"},
            "object_type": {3: "link", 4: "node", 5: "node"},
            "old_id": {3: "0", 4: "101982", 5: "101982"},
            "new_id": {3: "0", 4: "007", 5: "101982"},
            "old_attributes": {
    ...
    cols_to_compare = ["change_event", "object_type", "old_id", "new_id", "diff"]
    assert_frame_equal(
        network1.change_log[cols_to_compare].tail(3),
        correct_change_log_df[cols_to_compare],
        check_names=False,
        check_dtype=False,
    )
...
def test_reindex_link(network1):
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {3: "2020-06-08 19:34:48", 4: "2020-06-08 19:34:48"},
            "change_event": {3: "modify", 4: "modify"},
            "object_type": {3: "link", 4: "link"},
            "old_id": {3: "0", 4: "0"},
            "new_id": {3: "007", 4: "0"},
            "old_attributes": {
                3: "{'id': '0', 'from': '101982', 'to': '101986', 'freespeed': 4.166666666666667, 'capacity': 600.0, 'permlanes': 1.0, 'oneway': '1', 'modes': ['car'], 's2_from': 5221390329378179879, 's2_to': 5221390328605860387, 'length': 52.765151087870265, 'attributes': {'osm:way:access': {'name': 'osm:way:access', 'class': 'java.lang.String', 'text': 'permissive'}, 'osm:way:highway': {'name': 'osm:way:highway', 'class': 'java.lang.String', 'text': 'unclassified'}, 'osm:way:id': {'name': 'osm:way:id', 'class': 'java.lang.Long', 'text': '26997928'}, 'osm:way:name': {'name': 'osm:way:name', 'class': 'java.lang.String', 'text': 'Brunswick Place'}}}",
            ...
            "diff": {
                3: [("change", "id", ("0", "007")), ("change", "id", ("0", "007"))],
                4: [("change", "id", ("0", "007"))],
            },
        }
    )
...
def test_modify_node_adds_attributes_in_the_graph_and_change_is_recorded_by_change_log(
    ...
    cols_to_compare = ["change_event", "object_type", "old_id", "new_id", "diff"]
    assert_series_equal(
        n.change_log.loc[1, cols_to_compare],
        correct_change_log_df.loc[1, cols_to_compare],
        check_names=False,
        check_dtype=False,
    )
...
def test_modify_nodes_adds_and_changes_attributes_in_the_graph_and_change_is_recorded_by_change_log(
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {
                0: "2020-06-01 15:07:51",
                1: "2020-06-01 15:07:51",
                2: "2020-06-01 15:07:51",
                3: "2020-06-01 15:07:51",
            },
            "change_event": {0: "add", 1: "add", 2: "modify", 3: "modify"},
            "object_type": {0: "node", 1: "node", 2: "node", 3: "node"},
            "old_id": {0: None, 1: None, 2: 1, 3: 2},
    ...
    cols_to_compare = ["change_event", "object_type", "old_id", "new_id", "diff"]
    assert_frame_equal(
        n.change_log.loc[[2, 3], cols_to_compare],
        correct_change_log_df.loc[[2, 3], cols_to_compare],
        check_dtype=False,
    )
...
def test_apply_attributes_to_edge_without_filter_conditions():
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {2: "2020-07-10 14:53:25", 3: "2020-07-10 14:53:25"},
            "change_event": {2: "modify", 3: "modify"},
            "object_type": {2: "edge", 3: "edge"},
            "old_id": {2: "(1, 2, 0)", 3: "(1, 2, 1)"},
            "new_id": {2: "(1, 2, 0)", 3: "(1, 2, 1)"},
            "old_attributes": {
                2: "{'a': 1, 'from': 1, 'to': 2, 'id': '0'}",
                3: "{'b': 1, 'from': 1, 'to': 2, 'id': '1'}",
            },
            "new_attributes": {
                2: "{'a': 1, 'from': 1, 'to': 2, 'id': '0', 'c': 1}",
    ...
    assert_frame_equal(
        n.change_log[cols_to_compare].tail(2),
        correct_change_log_df[cols_to_compare],
        check_dtype=False,
    )
...
def test_apply_attributes_to_edge_with_filter_conditions():
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {2: "2020-07-10 14:53:25"},
            "change_event": {2: "modify"},
            "object_type": {2: "edge"},
            "old_id": {2: "(1, 2, 0)"},
            "new_id": {2: "(1, 2, 0)"},
    ...
    assert_frame_equal(
        n.change_log[cols_to_compare].tail(1),
        correct_change_log_df[cols_to_compare],
        check_dtype=False,
    )
...
def test_apply_attributes_to_multiple_edges_with_conditions():
    ...
    n.add_link("2", 2, 3, attribs={"c": 1})
    n.add_link("3", 2, 3, attribs={"d": 1})
    n.apply_attributes_to_edges(
        {(1, 2): {"e": 1}, (2, 3): {"f": 1}}, conditions=[{"a": (0, 2)}, {"c": (0, 2)}]
    )
...
def test_modify_link_adds_attributes_in_the_graph_and_change_is_recorded_by_change_log():
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {0: "2020-06-12 20:02:49", 1: "2020-06-12 20:02:49"},
            "change_event": {0: "add", 1: "modify"},
            "object_type": {0: "link", 1: "link"},
            "old_id": {0: None, 1: "0"},
            "new_id": {0: "0", 1: "0"},
            "old_attributes": {0: None, 1: "{'a': 1, 'from': 1, 'to': 2, 'id': '0'}"},
            "new_attributes": {
                0: "{'a': 1, 'from': 1, 'to': 2, 'id': '0'}",
                1: "{'a': 1, 'from': 1, 'to': 2, 'id': '0', 'b': 1}",
            },
            "diff": {
                0: [
                    ("add", "", [("a", 1), ("from", 1), ("to", 2), ("id", "0")]),
                    ("add", "id", "0"),
                ],
                1: [("add", "", [("b", 1)])],
            },
    ...
    assert_frame_equal(
        n.change_log[cols_to_compare], correct_change_log_df[cols_to_compare], check_dtype=False
    )
...
def test_modify_link_overwrites_existing_attributes_in_the_graph_and_change_is_recorded_by_change_log():
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {0: "2020-06-12 20:04:23", 1: "2020-06-12 20:04:23"},
            "change_event": {0: "add", 1: "modify"},
            "object_type": {0: "link", 1: "link"},
            "old_id": {0: None, 1: "0"},
            "new_id": {0: "0", 1: "0"},
...
def test_modify_links_adds_and_changes_attributes_in_the_graph_with_multiple_edges_and_change_is_recorded_by_change_log():
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {2: "2020-06-12 19:59:40", 3: "2020-06-12 19:59:40"},
            "change_event": {2: "modify", 3: "modify"},
            "object_type": {2: "link", 3: "link"},
            "old_id": {2: "0", 3: "1"},
            "new_id": {2: "0", 3: "1"},
            "old_attributes": {
                2: "{'a': {'b': 1}, 'from': 1, 'to': 2, 'id': '0'}",
                3: "{'c': 100, 'from': 1, 'to': 2, 'id': '1'}",
            },
            "new_attributes": {
                2: "{'a': {'b': 100}, 'from': 1, 'to': 2, 'id': '0'}",
...
def test_removing_single_node():
    ...
    correct_change_log = pd.DataFrame(
        {
            "timestamp": {4: "2020-06-11 10:37:54"},
            "change_event": {4: "remove"},
            "object_type": {4: "node"},
            "old_id": {4: 1},
            "new_id": {4: None},
            "old_attributes": {4: "{}"},
    ...
    assert_frame_equal(
        n.change_log[cols_to_compare].tail(1),
        correct_change_log[cols_to_compare],
        check_dtype=False,
    )
...
def test_removing_multiple_nodes():
    ...
    correct_change_log = pd.DataFrame(
        {
            "timestamp": {4: "2020-06-11 10:39:52", 5: "2020-06-11 10:39:52"},
            "change_event": {4: "remove", 5: "remove"},
            "object_type": {4: "node", 5: "node"},
            "old_id": {4: 1, 5: 2},
            "new_id": {4: None, 5: None},
            "old_attributes": {4: "{}", 5: "{}"},
...
def test_removing_single_link():
    ...
    correct_change_log = pd.DataFrame(
        {
            "timestamp": {4: "2020-06-12 19:58:01"},
            "change_event": {4: "remove"},
            "object_type": {4: "link"},
            "old_id": {4: "1"},
            "new_id": {4: None},
            "old_attributes": {4: "{'b': 4, 'from': 1, 'to': 2, 'id': '1'}"},
            "new_attributes": {4: None},
            "diff": {
                4: [
                    ("remove", "", [("b", 4), ("from", 1), ("to", 2), ("id", "1")]),
                    ("remove", "id", "1"),
                ]
            },
        }
...
def test_removing_multiple_links():
    ...
    assert "0" not in n.link_id_mapping
    assert "2" not in n.link_id_mapping

    correct_change_log = pd.DataFrame(
        {
            "timestamp": {4: "2020-06-12 19:55:10", 5: "2020-06-12 19:55:10"},
            "change_event": {4: "remove", 5: "remove"},
            "object_type": {4: "link", 5: "link"},
            "old_id": {4: "0", 5: "2"},
            "new_id": {4: None, 5: None},
            "old_attributes": {
                4: "{'a': 1, 'from': 1, 'to': 2, 'id': '0'}",
                5: "{'a': 1, 'from': 2, 'to': 3, 'id': '2'}",
            },
            "new_attributes": {4: None, 5: None},
            "diff": {
                4: [
                    ("remove", "", [("a", 1), ("from", 1), ("to", 2), ("id", "0")]),
                    ("remove", "id", "0"),
                ],
                5: [
                    ("remove", "", [("a", 1), ("from", 2), ("to", 3), ("id", "2")]),
                    ("remove", "id", "2"),
                ],
            },
        }
...
def test_schedule_routes_with_disconnected_routes(network_object_from_test_data):
    ...
    n.schedule.apply_attributes_to_routes(
        {"VJbd8660f05fe6f744e58a66ae12bd66acbca88b98": {"network_route": ["1", "2"]}}
    )
...
def test_read_matsim_network_with_duplicated_node_ids_records_removal_in_changelog(mocker):
    ...
    mocker.patch.object(
        matsim_reader, "read_network", return_value=(nx.MultiDiGraph(), 2, dup_nodes, {}, {})
    )
    network = read.read_matsim(path_to_network=pt2matsim_network_test_file, epsg="epsg:27700")

    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {0: "2020-07-02 11:36:54"},
    ...
    assert_frame_equal(
        network.change_log[cols_to_compare].tail(1),
        correct_change_log_df[cols_to_compare],
        check_names=False,
        check_dtype=False,
    )
...
def test_read_matsim_network_with_duplicated_link_ids_records_reindexing_in_changelog(mocker):
    ...
    correct_change_log_df = pd.DataFrame(
        {
            "timestamp": {0: "2020-07-02 11:59:00"},
            "change_event": {0: "modify"},
            "object_type": {0: "link"},
            "old_id": {0: "1"},
            "new_id": {0: "1_1"},
            "old_attributes": {0: "{'heyooo': '1'}"},
            "new_attributes": {0: "{'heyooo': '1'}"},
            "diff": {0: [("change", "id", ("1", "1_1"))]},
        }
    )
    cols_to_compare = [
        "change_event",
            ...
            Service(
                ...
                routes=[
                    Route(
                        ...
                        mode="bus",
                        stops=[
                            Stop(id="0", x=1, y=1, epsg="epsg:4326"),
                            Stop(id="1", x=2, y=2, epsg="epsg:4326"),
                        ],
                        trips={
                            "trip_id": ["trip-04:40:00"],
                            "trip_departure_time": ["04:40:00"],
                            "vehicle_id": ["veh_1_bus"],
                        },
                        network_route=["l1", "l2"],
    ...
    correct_gdf = gpd.GeoDataFrame(
        {
            ...
            "route_id": {0: "service_0"},
            "mode": {0: "bus"},
            "route_short_name": {0: "route"},
            "geometry": {0: LineString([(1, 1), (2, 2), (1, 1)])},
        }
    ).set_crs(n.epsg)
...
@pytest.fixture()
def network_cases_for_testing_isolated_nodes(
    network_with_isolated_nodes, network_without_isolated_nodes
):
    return {
...
def test_networks_report_correct_isolated_nodes(
    network_case, network_cases_for_testing_isolated_nodes
):
    assert (
        network_cases_for_testing_isolated_nodes[network_case]["network"].isolated_nodes()
...
def test_isolated_nodes_show_up_in_validation_report(
    ...
    assert_semantically_equal(
        report["graph"]["isolated_nodes"], {"number_of_nodes": 1, "nodes": ["1"]}
    )
...
def test_has_schedule_with_valid_network_routes_with_valid_routes(simple_route):
    ...
    n.add_link("1", 1, 2, attribs={"modes": ["bus"]})
    n.add_link("2", 2, 3, attribs={"modes": ["car", "bus"]})
    simple_route.network_route = ["1", "2"]
    ...
    n.schedule.apply_attributes_to_routes(
        {"service_0": {"network_route": ["1", "2"]}, "service_1": {"network_route": ["1", "2"]}}
    )
...
def test_has_schedule_with_valid_network_routes_with_invalid_routes(simple_route):
    ...
    n.add_link("1", 1, 2)
    n.add_link("2", 2, 3)
    simple_route.network_route = ["3", "4"]
...
def test_invalid_network_routes_with_valid_route(simple_route):
    ...
    n.schedule = Schedule(n.epsg, [Service(id="service", routes=[simple_route])])
    n.schedule.apply_attributes_to_routes({"route": {"network_route": ["1", "2"]}})
...
def test_invalid_network_routes_with_invalid_route(simple_route):
    ...
    simple_route.reindex("route")
    n.schedule = Schedule(n.epsg, [Service(id="service", routes=[simple_route])])
    n.schedule.apply_attributes_to_routes({"route": {"network_route": ["3", "4"]}})
...
def test_invalid_network_routes_with_empty_route(simple_route):
    ...
    simple_route.reindex("route")
    n.schedule = Schedule(n.epsg, [Service(id="service", routes=[simple_route])])
    n.schedule.apply_attributes_to_routes({"route": {"network_route": []}})
    assert n.invalid_network_routes() == ["route"]
...
def invalid_pt2matsim_network_for_validation(network_object_from_test_data):
    return {
        ...
        "subgraph_no_per_mode": {"car": 2, "walk": 2, "bike": 0},
...
def test_connectivity_in_report_with_invalid_network(invalid_pt2matsim_network_for_validation):
    ...
    for mode, expected_connected_subgraphs in invalid_pt2matsim_network_for_validation[
        "subgraph_no_per_mode"
    ].items():
        assert (
            report["graph"]["graph_connectivity"][mode]["number_of_connected_subgraphs"]
            == expected_connected_subgraphs
        )
...
def test_schedule_validity_in_report_with_invalid_network(invalid_pt2matsim_network_for_validation):
    ...
    assert (
        ...
        == invalid_pt2matsim_network_for_validation["is_valid_schedule"]
    )


def test_invalid_service_identified_in_report_with_invalid_network(
...
def test_network_routing_in_report_with_invalid_network(invalid_pt2matsim_network_for_validation):
    report = invalid_pt2matsim_network_for_validation["network"].generate_validation_report()
    assert (
        report["routing"]["services_have_routes_in_the_graph"]
        == invalid_pt2matsim_network_for_validation["valid_PT_network_routes"]
    )


def test_invalid_network_routes_show_in_report_with_invalid_network(
    invalid_pt2matsim_network_for_validation,
):
    report = invalid_pt2matsim_network_for_validation["network"].generate_validation_report()
    route_ids_with_invalid_network_route = invalid_pt2matsim_network_for_validation[
        "pt_routes_with_invalid_network_route"
    ]
    assert (
        report["routing"]["service_routes_with_invalid_network_route"]
...
def valid_network_for_validation(correct_schedule):
    ...
    return {
        "network": n,
        "subgraph_no_per_mode": {"car": 1, "walk": 0, "bike": 0},
...
def test_connectivity_in_report_with_valid_network(valid_network_for_validation):
    ...
    for mode, expected_connected_subgraphs in valid_network_for_validation[
        "subgraph_no_per_mode"
    ].items():
        assert (
            report["graph"]["graph_connectivity"][mode]["number_of_connected_subgraphs"]
...
def test_schedule_validity_in_report_with_valid_network(valid_network_for_validation):
    ...
    assert (
        report["schedule"]["schedule_level"]["is_valid_schedule"]
        == valid_network_for_validation["is_valid_schedule"]
    )
...
def test_network_routing_in_report_with_valid_network(valid_network_for_validation):
    ...
    assert (
        report["routing"]["services_have_routes_in_the_graph"]
        == valid_network_for_validation["has_valid_PT_network_routes"]
    )
...
def test_long_links_show_up_in_validation_report(assert_semantically_equal):
    ...
    n.add_link(
        "2", 2, 3, attribs={"length": 2, "capacity": 1, "freespeed": 2, "modes": ["car", "bus"]}
    )
...
def test_values_of_ids_are_not_flagged_in_validation_report(
    ...
    n.add_link(
        ...
        1,
        2,
        attribs={"length": 1, "capacity": 1, "freespeed": 1, "modes": ["car", "bus"]},
    )
    n.add_link(
        "2", 2, 3, attribs={"length": 2, "capacity": 1, "freespeed": 2, "modes": ["car", "bus"]}
    )
...
def test_zero_value_attributes_show_up_in_validation_report(assert_semantically_equal):
    ...
    n.add_link(
        ...
        1,
        2,
        attribs={"length": 0, "capacity": 0.0, "freespeed": "0.0", "modes": ["car", "bus"]},
    )
    n.add_link(
        "2", 2, 3, attribs={"length": 2, "capacity": 1, "freespeed": 2, "modes": ["car", "bus"]}
    )
...
def test_fractional_value_attributes_show_up_in_validation_report(assert_semantically_equal):
    ...
    n.add_link(
        ...
        1,
        2,
        attribs={"length": 0.1, "capacity": "0.0", "freespeed": "0.2", "modes": ["car", "bus"]},
    )
    n.add_link(
        "2", 2, 3, attribs={"length": 2, "capacity": 1, "freespeed": 2, "modes": ["car", "bus"]}
    )
...
def test_none_value_attributes_show_up_in_validation_report(assert_semantically_equal):
    ...
    n.add_link(
        ...
        1,
        2,
        attribs={"length": 1, "capacity": "None", "freespeed": None, "modes": ["car", "bus"]},
    )
    n.add_link(
        "2", 2, 3, attribs={"length": 2, "capacity": 1, "freespeed": 2, "modes": ["car", "bus"]}
    )
...
def test_check_connectivity_for_mode_warns_of_graphs_with_more_than_single_component(
    ...
    mocker.patch.object(
        ...
        return_value={
            "problem_nodes": {"dead_ends": [], "unreachable_node": ["1"]},
...
def test_write_to_matsim_generates_network_matsim_file_if_network_is_car_only(
...
def test_reindexing_network_node_with_auxiliary_files(aux_network):
    ...
    assert aux_network.auxiliary_files["node"]["links_benchmark.csv"].map == {
        "2": "2",
        "3": "0",
        "4": "4",
        "1": "1",
...
def test_saving_network_with_auxiliary_files_with_changes(aux_network, tmpdir):
    aux_network.auxiliary_files["node"]["links_benchmark.csv"].map = {
        "2": None,
        "3": None,
        "4": "04",
        "1": None,
    }
    aux_network.auxiliary_files["link"]["links_benchmark.json"].map = {
        "2": "002",
        "1": "001",
        "3": "003",
        "4": "004",
    ...
    assert pd.read_csv(expected_csv_aux_file)["links"].to_dict() == {
        0: "[None]",
        1: "[None]",
        2: "[None]",
        3: "['04']",
    ...
    nodes = gpd.GeoDataFrame(
        {
            ...
            "101986": {
                ...
                "lat": 51.52228713323965,
                "s2_id": 5221390328605860387,
                "geometry": Point(528835.203274008, 182006.27331298392),
            },
        }
    ).T
...
def test_transforming_uneven_network_to_json(assert_semantically_equal):
    ...
    n.add_link(
        ...
        "101982",
        "101986",
        attribs={"id": "0", "from": "101982", "to": "101986", "freespeed": 4},
    )
    n.add_link(
        "0",
        "101982",
        "101986",
        attribs={"id": "0", "from": "101982", "to": "101986", "capacity": 5},
    )
...
def test_saving_network_to_json(assert_semantically_equal, network_1_geo_and_json, tmpdir):
    ...
    assert_semantically_equal(output_json, network_1_geo_and_json["expected_sanitised_json"])
...
def test_transforming_network_to_geodataframe(network_1_geo_and_json):
    ...
    assert_frame_equal(
        _network["nodes"][node_cols],
        network_1_geo_and_json["expected_geodataframe"]["nodes"][node_cols],
        check_dtype=False,
    )
    assert set(_network["links"].columns) == set(link_cols)
    assert_frame_equal(
        _network["links"][link_cols],
        network_1_geo_and_json["expected_geodataframe"]["links"][link_cols],
        check_dtype=False,
    )
...
def test_saving_network_to_csv(assert_semantically_equal, network1, correct_schedule, tmpdir):
    ...
    assert set(os.listdir(os.path.join(tmpdir, "network"))) == {
        "nodes.csv",
        "links.csv",
        "network_change_log.csv",
    }
    ...
    assert_semantically_equal(
        ...
        {
            "index": {0: 101982, 1: 101986},
            "lat": {0: 51.52287873323954, 1: 51.52228713323965},
            "s2_id": {0: 5221390329378179879, 1: 5221390328605860387},
            "lon": {0: -0.14625948709424305, 1: -0.14439428709377494},
            "id": {0: 101982, 1: 101986},
            "x": {0: 528704.1425925883, 1: 528835.203274008},
            "geometry": {
                0: "[528704.1425925883, 182068.78193707118]",
    ...
    assert_semantically_equal(
        output_links.to_dict(),
        {
            ...
            "modes": {0: "['car']"},
            "to": {0: 101986},
            "s2_from": {0: 5221390329378179879},
            "length": {0: 52.76515108787025},
            "id": {0: 0},
            "from": {0: 101982},
            "s2_to": {0: 5221390328605860387},
            "capacity": {0: 600.0},
            "u": {0: 101982},
            "v": {0: 101986},
...
def test_splitting_link_at_point_gets_data_right(mocker):
    ...
    mocker.patch.object(
        Network, "generate_indices_for_n_edges", return_value=(new_link_1_ID, new_link_2_ID)
    )

    n = Network("epsg:27700")
    n.add_nodes(
        {"n1": {"id": "n1", "x": 528568, "y": 177243}, "n2": {"id": "n2", "x": 528570, "y": 177243}}
    )
    n.add_links(
        {
            "l1": {
    ...
    assert {k: v for k, v in data["node_attributes"].items() if k in ["id", "x", "y"]} == {
        "id": new_node_ID,
        "x": 528568.5,
        "y": 177243.0,
    }
    assert list(data["links"][new_link_1_ID].pop("geometry").coords) == [
        (528568, 177243),
        (528568.5, 177243),
    ]
    assert data["links"][new_link_1_ID] == {
        ...
        "modes": ["car"],
        "length": 2.5,
        "s2_from": n.node("n1")["s2_id"],
        "s2_to": data["node_attributes"]["s2_id"],
        "attributes": {
            "osm:way:access": {
                "name": "osm:way:access",
                "class": "java.lang.String",
    ...
    assert list(data["links"][new_link_2_ID].pop("geometry").coords) == [
        (528568.5, 177243),
        (528569, 177243),
        (528570, 177243),
    ]
    assert data["links"][new_link_2_ID] == {
        ...
        "modes": ["car"],
        "length": 7.5,
        "s2_from": data["node_attributes"]["s2_id"],
        "s2_to": n.node("n2")["s2_id"],
        "attributes": {
            "osm:way:access": {
                "name": "osm:way:access",
                "class": "java.lang.String",
...
def test_splitting_link_with_existing_node_id_generates_new_index(mocker):
    ...
    n.split_link_at_point("l1", 528568.5, 177243, suggested_node_id)
...
def test_splitting_link_updates_route_in_schedule(mocker):
            ...
            Service(
                ...
                routes=[
                    Route(
                        ...
                        mode="bus",
                        stops=[
                            Stop(id="0", x=528568, y=177243, epsg="epsg:27700", linkRefId="A"),
                            Stop(id="1", x=528570, y=177243, epsg="epsg:27700", linkRefId="B"),
                        ],
                        trips={
                            "trip_id": ["trip_04:40:00"],
                            "trip_departure_time": ["04:40:00"],
                            "vehicle_id": ["veh_1_bus"],
                        },
                        arrival_offsets=["00:00:00", "00:02:00"],
                        departure_offsets=["00:00:00", "00:02:00"],
                        network_route=["AAA", "l1", "BBB"],
                    )
                ],
            )
        ],
    ...
    assert n.schedule.route("1").network_route == ["AAA", new_link_1_ID, new_link_2_ID, "BBB"]

...
class ScheduleElement:
    ...
    def headway_stats(self, from_time=None, to_time=None, gtfs_day="19700101"):
        ...
        if not df.empty:
            ...
            df_stats = route_groups.describe()
            df_headway = df_stats["headway_mins"][["mean", "std", "max", "min"]]
            df_headway["trip_count"] = route_groups.apply(len)
            df_headway.reset_index(inplace=True)
            df_headway = df_headway.rename(
                columns={
...
class Schedule(ScheduleElement):
    ...
    def _find_stops_on_shapely_geometry(self, shapely_input):
        stops_gdf = self.to_geodataframe()["nodes"].to_crs(EPSG4326)

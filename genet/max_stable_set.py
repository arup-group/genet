...
class MaxStableSet:
    def __init__(self, pt_graph, network_spatial_tree, modes, distance_threshold=30, step_size=10):
        ...
        self.stops = _gdf["nodes"].to_crs(EPSG4326)
        self.pt_edges = _gdf["links"].to_crs(EPSG4326)
    ...
    def _generate_artificial_path(self, from_link, to_link):
        return [
            from_link,
            self._generate_artificial_link(
                self._access_link_data(from_link)["to"], self._access_link_data(to_link)["from"]
            ),
            to_link,
        ]
...
class ChangeSet:
    ...
    def generate_additional_links_modes(self, max_stable_set):
        link_ids = {
            link_id for route_list in self.df_route_data["route"].values for link_id in route_list
        }

...
class Network:
    def __init__(self, epsg, **kwargs):
        self.epsg = CRS(epsg)
        self.transformer = Transformer.from_crs(epsg, EPSG4326, always_xy=True)
        self.graph = nx.MultiDiGraph(name="Network graph", crs=CRS(epsg))
    ...
    def reproject(self, new_epsg, processes=1):
        ...
        gdf_geometries = gdf_geometries.to_crs(CRS(new_epsg))
    ...
    def connect_components(self, modes: Union[list, str, set] = None, weight: float = 1.0):
        ...
        if len(components) == 1:
            ...
            gdf = gdfs["nodes"].to_crs(EPSG4326)
            components_gdfs = [
                        ...
                        for route_group, graph_group in zip(routes, graph_groups):
                            try:
                                ...
                                if changeset is None:
                                    changeset = mss.to_changeset(
                                        route_data.loc[list(route_group), :]
                                    )
                                else:
                                    changeset += mss.to_changeset(
                                        route_data.loc[list(route_group), :]
    ...
    def write_to_geojson(self, output_dir, epsg: str = None):
        ...
        if epsg is not None:
            _network["nodes"] = _network["nodes"].to_crs(CRS(epsg))
            _network["links"] = _network["links"].to_crs(CRS(epsg))

...
def generate_standard_outputs_for_schedule(
    ...
    schedule_links = schedule.to_geodataframe()["links"].to_crs(EPSG4326)
    ...
    for mode in schedule.modes():
        ...
        save_geodataframe(
            schedule_subgraph["links"].to_crs(EPSG4326),
            filename=f"schedule_subgraph_links_{mode}",
            output_dir=subgraph_dir,
            include_shp_files=include_shp_files,
        )
        save_geodataframe(
            schedule_subgraph["nodes"].to_crs(EPSG4326),
...
def generate_standard_outputs(
    ...
    graph_links = n.to_geodataframe()["links"].to_crs(EPSG4326)
    ...
    if n.schedule:
        ...
        gdf_routes = n.schedule_network_routes_geodataframe().to_crs(EPSG4326)
        save_geodataframe(
            gdf_routes,

...
def plot_graph(g, filename, ax=None, show=True, save=False, output_dir='', e_c='#999999'):
    if output_dir:
        filepath = os.path.join(output_dir, filename + '.png')
    else:
        filepath = None
    return ox.plot_graph(
        g, filepath=filepath, node_size=2, edge_color=e_c, save=save, show=show, ax=ax)


def plot_graph_routes(g, routes, filename, ax=None, show=True, save=False, output_dir=''):
    if output_dir:
        filepath = os.path.join(output_dir, filename + '.png')
    else:
        filepath = None
    if len(routes) == 0:
        ...
        return ox.plot_graph_routes(
            ...
            filepath=filepath, save=save, show=show, ax=ax)

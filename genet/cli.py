...
def add_elevation_to_network(
    ...
    elevation = network.get_node_elevation_dictionary(
        elevation_tif_file_path=path_to_elevation, null_value=null_value
    )
    ...
    report = elevation.validation_report_for_node_elevation(elevation)
...
def auto_schedule_fixes(
    ...
    if vehicle_scalings is not None:
        ...
        _write_scaled_vehicles(network, vehicle_scalings, output_dir)
...
def find_closest_links_by_step(
...
def generate_modal_network_geojsons(network, modes, output_dir, filename_suffix):
    ...
    for mode in modes:
        ...
        _gdf["modes"] = _gdf["modes"].apply(lambda x: ",".join(list(x).sort()))
...
def separate_modes_in_network(
    ...
    Examples:
        ...
        In the case when a link already has a single dedicated mode, no updates are made to the link ID, you can assume that all links that were in the network previously are still there, but their allowed modes may have changed, so any simulation outputs may not be valid with this new network.
    """
    modes = modes.split(",")
    supporting_outputs = output_dir / "supporting_outputs"
    ensure_dir(output_dir)
    ...
    generate_modal_network_geojsons(network, modes, supporting_outputs, "before")
    ...
    generate_modal_network_geojsons(network, modes, supporting_outputs, "after")
...
def squeeze_external_area(
    ...
    network_internal = gpd.sjoin(network_gdf, gdf_study_area, how="inner", predicates="intersects")
...
def squeeze_urban_links(
    ...
    if path_to_study_area is not None:
        ...
        gdf_urban = gpd.sjoin(gdf_urban, gdf_study_area, how="inner", predicates="intersects").drop(
            columns=["index_right"]
        )

    logging.info("Finding urban links")
    network_gdf = network.to_geodataframe()["links"]
    network_urban = gpd.sjoin(network_gdf, gdf_urban, how="inner", predicates="intersects").drop(
        columns=["index_right"]
    )
    if path_to_study_area is not None:
        ...
        network_urban = gpd.sjoin(network_gdf, gdf_study_area, how="inner", predicates="intersects")
